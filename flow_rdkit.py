...
G0 = gaussian_cal.GaussianCal(method='B3LYP',basis='6-311G(d)',charge='pos',wfn=True,debug=True)
for i in range(len(name_list)):
    utils.smile2xyz(f'{name_list[i]}_0.xyz',smiles_list[i])
    G0.Run(f'{name_list[i]}_0.xyz')

G1 = gaussian_cal.GaussianCal(method='B3LYP',basis='6-311G(d)',charge='neu',wfn=True,debug=True)
for i in range(len(name_list)):
    #提取上次高斯之后的坐标
